...
class Radian(models.Model):
    ...
    event_id = fields.Many2one(
        comodel_name="l10n_co_edi_shf.events",
        string="Event",
        required=True,
        readonly=True,
        tracking=True,
    ...
    rejection_concept_id = fields.Many2one(
        comodel_name="l10n_co_edi_shf.rejection_concepts",
        string="Rejection concept",
        required=False,
        readonly=True,
        ondelete="RESTRICT",
        tracking=True,
    ...
    edi_status_description = fields.Char(
        "Status description", copy=False, readonly=True
    )
    ...
    edi_application_response_base64 = fields.Binary(
        "Application response", copy=False, readonly=True
    )
    edi_attached_document_base64 = fields.Binary(
        "Attached document", copy=False, readonly=True
    )
    ...
    # For mail attached
    edi_attached_zip_base64 = fields.Binary(
    ...
    type = fields.Selection(
        ...
        index=True,
        change_default=True,
        default=lambda self: self._context.get("type", "customer"),
        tracking=True,
    )
    ...
    def _send_email(self):
        for rec in self:
            mail_template = rec.env.ref("l10n_co_edi_shf.email_template_radian", False)
            ctx = dict(active_model="l10n_co_edi_shf.radian")
            if mail_template:
    ...
    def action_send_email(self):
        self.ensure_one()
        mail_template = self.env.ref("l10n_co_edi_shf.email_template_radian", False)
        ctx = dict(
            default_model="l10n_co_edi_shf.radian",
        ...
        return {
            ...
            "src_model": "l10n_co_edi_shf.radian",
            "view_mode": "form",
            "target": "new",
            "view_type": "form",
            "views": [(False, "form")],
    ...
    def action_post(self):
        for rec in self:
            ...
            if rec.type == "customer" and rec.move_id.move_type not in (
                "out_invoice",
                "out_refund",
            ):
                raise UserError(_("The invoice must be a sales invoice"))
            if rec.type == "supplier" and rec.move_id.move_type not in (
                "in_invoice",
                "in_refund",
            ):
            ...
            seq_search = self.env["ir.sequence"].search(
                [("code", "=", name_sequence), ("company_id", "=", rec.company_id.id)]
            )
            if not seq_search:
                seq_search = self.env["ir.sequence"].search(
                    [("code", "=", name_sequence)]
            ...
            if (
                rec.name
                and rec.name not in ("New", _("New"))
                and rec.name[0 : len(prefix)] == prefix
            ):
                rec.number = "".join(
                    [i for i in rec.name[len(prefix) :] if i.isdigit()]
                )
                rec.prefix = prefix
            else:
    ...
    def unlink(self):
        for rec in self:
            if rec.state not in ("draft", "cancel"):
                raise UserError(
                    _("You cannot delete an event which is not draft or cancelled.")
                )
            elif rec.name and rec.name not in ("New", _("New")):
                raise UserError(
                    _(
                        "You cannot delete an event after it has been posted (and received a number). "
    ...
    def get_json_request(self):
        for rec in self:
            if rec.event_id.code == "031" and not rec.rejection_concept_id:
                raise UserError(
                    _("The rejection concept is required for the DIAN claim event.")
                )
            if not rec.move_id.ei_uuid:
                raise UserError(
                    _("The invoice UUID (CUFE) is required for DIAN events.")
                )
            if not rec.user_id.type_document_identification_id:
                raise UserError(
                    _("The document type for user is required for DIAN events.")
                )
            if not rec.user_id.vat:
                raise UserError(
                    _("The document number (VAT) for user is required for DIAN events.")
                )
            ...
            if not rec.user_id or not rec.user_id.function:
                raise UserError(
                    _(
                        "The user and his/her job title (function) is required for DIAN events"
                    )
                )
    ...
    def validate_dian(self):
        for rec in self:
            ...
            if (
                rec.company_id.ei_enable
                and not rec.edi_is_valid
                and (
                    (
                        rec.type == "supplier"
                        and rec.event_id.code in ("030", "031", "032", "033")
                    )
                    or (rec.type == "customer" and rec.event_id.code == "034")
                )
            ):
    ...
    def validate_dian_generic(self):
        for rec in self:
            try:
                ...
                params = {"token": token, "code": rec.event_id.code}
                header = {
    ...
    def button_open_form_current(self):
        view = self.env.ref("l10n_co_edi_shf.view_l10n_co_edi_shf_radian_form")
        ...
        return {
            ...
            "view_type": "form",
            "view_mode": "form",
            "res_model": "l10n_co_edi_shf.radian",
            "views": [(view.id, "form")],
            "view_id": view.id,
            "target": "current",
            "res_id": rec_id,
