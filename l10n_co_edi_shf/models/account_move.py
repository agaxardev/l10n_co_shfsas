...
class AccountMove(models.Model):
    ...
    state = fields.Selection(
        selection_add=[("validate", "Validating DIAN")],
        ondelete={"validate": "set default"},
    )
    number_formatted = fields.Char(
        string="Number formatted",
        compute="compute_number_formatted",
        store=True,
        copy=False,
    )

    ei_type_document_id = fields.Many2one(
        comodel_name="l10n_co_edi_shf.type_documents",
    ...
    ei_application_response_base64_bytes = fields.Binary(
        "Application response", attachment=True, copy=False, readonly=True
    )
    ei_attached_document_base64_bytes = fields.Binary(
        "Attached document", attachment=True, copy=False, readonly=True
    )
    ei_pdf_base64_bytes = fields.Binary(
        "Pdf document", attachment=True, copy=False, readonly=True
    )
    ei_zip_base64_bytes = fields.Binary(
        "Zip document", attachment=True, copy=False, readonly=True
    )
    ei_type_environment = fields.Many2one(
        comodel_name="l10n_co_edi_shf.type_environments",
    ...
    ei_dian_response_base64_bytes = fields.Binary(
        "DIAN response", attachment=True, copy=False, readonly=True
    )

    # For mail attached
    ei_attached_zip_base64_bytes = fields.Binary(
        "Attached zip", attachment=True, copy=False, readonly=True
    )
    ...
    ei_amount_total_no_withholding = fields.Monetary(
        "Total without withholdings", compute="_compute_amount", store=True
    )
    ...
    ei_correction_concept_id = fields.Many2one(
        comodel_name="l10n_co_edi_shf.correction_concepts",
        string="Correction concept",
        copy=False,
        readonly=True,
        compute="compute_ei_correction_concept_id",
        store=True,
        ondelete="RESTRICT",
    )
    ei_correction_concept_credit_id = fields.Many2one(
        comodel_name="l10n_co_edi_shf.correction_concepts",
        string="Credit correction concept",
        copy=False,
        readonly=True,
        domain=[("type_document_id", "=", "5")],
        ondelete="RESTRICT",
    )
    ei_correction_concept_debit_id = fields.Many2one(
        comodel_name="l10n_co_edi_shf.correction_concepts",
        string="Debit correction concept",
        copy=False,
        readonly=True,
        domain=[("type_document_id", "=", "6")],
        ondelete="RESTRICT",
    )
    ei_is_correction_without_reference = fields.Boolean(
        "Is it a correction without reference?", default=False, readonly=True
    )
    ...
    is_attached_document_matched = fields.Boolean(
        "Correct number in attached document?",
        copy=False,
        compute="_is_attached_document_matched",
        store=True,
    )
    ei_operation = fields.Selection(
        ...
        default=lambda self: self.env.company.ei_operation,
        copy=True,
        readonly=True,
        required=True,
    )
    ...
    date_start = fields.Date(
        string="Start date", default=None, copy=True, readonly=True
    )
    date_end = fields.Date(string="End date", default=None, copy=True, readonly=True)

    # Order Reference
    order_ref_number = fields.Char(
        string="Order reference", default=None, copy=False, readonly=True
    )
    order_ref_date = fields.Date(
        string="Order date", default=None, copy=False, readonly=True
    )

    # Is out of country
    is_out_country = fields.Boolean(
        string="Is it for out of the country?",
    ...
    payment_form_id = fields.Many2one(
        string="Payment form",
        comodel_name="l10n_co_edi_shf.payment_forms",
        copy=True,
        store=True,
        compute="_compute_payment_form_id",
        readonly=True,
        ondelete="RESTRICT",
    )
    payment_method_id = fields.Many2one(
        string="Payment method",
        comodel_name="l10n_co_edi_shf.payment_methods",
    ...
    resolution_id = fields.Many2one(
        string="Resolution",
        comodel_name="l10n_co_edi_shf.resolution",
        copy=False,
        store=True,
    ...
    radian_ids = fields.One2many(
        comodel_name="l10n_co_edi_shf.radian", inverse_name="move_id"
    )

    is_edi_mail_sent = fields.Boolean(
        readonly=True,
        default=False,
        copy=False,
        ...
        for rec in self:
            if rec.ei_uuid:
                return {
                    "type": "ir.actions.act_url",
                    "target": "new",
                    "url": "https://catalogo-vpfe.dian.gov.co/Document/DownloadPDF?trackId="
                    + rec.ei_uuid,
                }
    ...
    def _send_edi_email(self):
        for rec in self:
            mail_template = rec.env.ref("l10n_co_edi_shf.email_template_edi", False)
    ...
    def _default_ei_type_environment(self):
        if not self.env["l10n_co_edi_shf.type_environments"].search_count([]):
            self.env["res.company"].init_csv_data(
                "l10n_co_edi_shf.l10n_co_edi_shf.type_environments"
            )
    ...
    def _default_payment_method_id(self):
        if not self.env["l10n_co_edi_shf.payment_methods"].search_count([]):
            self.env["res.company"].init_csv_data(
                "l10n_co_edi_shf.l10n_co_edi_shf.payment_methods"
            )
        return 1

    @api.depends("invoice_date", "invoice_date_due")
    def _compute_payment_form_id(self):
        if not self.env["l10n_co_edi_shf.payment_forms"].search_count([]):
            self.env["res.company"].init_csv_data(
                "l10n_co_edi_shf.l10n_co_edi_shf.payment_forms"
            )
    ...
    def is_universal_discount(self):
        try:
            if "ks_amount_discount" in self:
                return bool(
                    self.env["res.company"].search([("ks_enable_discount", "=", True)])
                )
            else:
                return False
        except KeyError:
    ...
    def write_response(self, response, payload):
        try:
            for rec in self:
                ...
                rec.ei_application_response_base64_bytes = response[
                    "application_response_base64_bytes"
                ]
                rec.ei_attached_document_base64_bytes = response[
    ...
    def get_ei_customer(self):
        for rec in self:
            ...
            if type_document_identification_id:
                if rec.partner_id.vat:
                    ...
                    if identification_number:
                        ...
                        if rec_partner.country_id:
                            customer_rec = self.env["l10n_co_edi_shf.countries"].search(
                        ...
                        if rec_partner.country_id.code == "CO":
                            if rec.is_out_country:
                                raise UserError(
                                    _(
                                        "This is an export invoice but the client's country is Colombia"
                                    )
                                )

                            if rec_partner.municipality_id:
                                customer_data["municipality_code"] = (
                                    rec_partner.municipality_id.id
                                )
                            elif rec_partner.postal_municipality_id:
                                municipality_rec = self.env[
                                    "l10n_co_edi_shf.municipalities"
                                ].search(
                                    [
                                        (
                                            "code",
                                            "=",
                                            rec_partner.postal_municipality_id.code,
                                        )
                                    ]
                                )
                        ...
                        if rec_partner.type_liability_id:
                            customer_data["liability_code"] = (
                                rec_partner.type_liability_id.id
                            )
                        else:
                            raise UserError(
                ...
                raise UserError(
                    _(
                        "The client does not have an associated identification document type."
                    )
                )
        return False
    ...
    def get_ei_legal_monetary_totals(self):
        ...
        tax_inclusive_amount = (
            payable_amount - charge_total_amount + allowance_total_amount
        )
            ...
            for invoice_line_id in rec.invoice_line_ids:
                if invoice_line_id.account_id:
                    if not (0 <= invoice_line_id.discount < 100):
                        raise UserError(
                            _(
                                "The discount must always be greater than or equal to 0 and less than 100."
                            )
                        )
                    ...
                    if (
                        not taxable_amount
                        and not discount
                        and invoice_line_id.product_id.lst_price
                    ):
                        ...
                        taxable_amount = (
                            invoice_line_id.product_id.lst_price
                            * invoice_line_id.quantity
                        )
                        products.update(
                            {"price_value": invoice_line_id.product_id.lst_price}
                        )
                    ...
                    for invoice_line_tax_id in invoice_line_id.tax_ids:
                        ...
                        if invoice_line_tax_id.edi_tax_id.id:
                            ...
                            dian_report_tax_base = (
                                invoice_line_tax_id.dian_report_tax_base or "auto"
                            )
                            # The information sent to DIAN should not include the withholdings
                            if (
                                edi_tax_name[:4] != "Rete"
                                and not tax_name.startswith(
                                    ("IVA Excluido", "IVA Compra Excluido")
                                )
                                and not (
                                    edi_tax_name == "IVA"
                                    and dian_report_tax_base == "no_report"
                                )
                            ):
                                if invoice_line_tax_id.amount_type == "percent":
                                    ...
                                    tax_total.update(
                                        {
                                            "tax_value": round_curr(
                                                taxable_amount
                                                * invoice_line_tax_id.amount
                                                / 100.0
                                            )
                                        }
                                    )
                                    tax_total.update(
                                        {"taxable_value": round_curr(taxable_amount)}
                                    )
                                    tax_total.update(
                                    ...
                                    if commercial_sample:
                                        ...
                                        tax_total.update(
                                            {
                                                "percent": invoice_line_id.price_total
                                                / taxable_amount
                                                * 100
                                            }
                                        )
                                    ...
                                    raise UserError(
                                        _(
                                            "Electronic invoicing is not yet compatible with this tax type."
                                        )
                                    )
                        else:
                            raise UserError(
                    ...
                    if rec.ei_operation == "transport":
                        if "waypoint_id" not in invoice_line_id:
                            raise UserError(
                                _(
                                    "Transport compatibility is only available with "
                                    "shf SAS Freight Transport Module"
                                )
                            )
                        ...
                        if invoice_line_id.waypoint_id:
                            ...
                            if not invoice_line_id.waypoint_id.total:
                                raise UserError(
                                    _("The waypoint %s doesn't have an total")
                                    % invoice_line_id.waypoint_id.name_seq
                                )
                            if not invoice_line_id.waypoint_id.weight:
                                raise UserError(
                                    _("The waypoint %s doesn't have an weight")
                                    % invoice_line_id.waypoint_id.name_seq
                                )
        ...
        for rec in self:
            ...
            for invoice_line_id in rec.invoice_line_ids:
                if invoice_line_id.account_id:
                        ...
                        if invoice_line_tax_id.edi_tax_id.id:
                            ...
                            if tax_name.startswith(
                                ...
                                amount_tax_withholding = (
                                    amount_tax_withholding + tax_amount
                                )
                            else:
                                amount_tax_no_withholding = (
                                    amount_tax_no_withholding + tax_amount
                                )
                        else:
                            if tax_name.startswith(
                                ("IVA Excluido", "IVA Compra Excluido")
                            ) or (
                                tax_name.startswith("IVA")
                                and dian_report_tax_base == "no_report"
                            ):
                                amount_excluded = amount_excluded + taxable_amount
                            elif tax_name[:3] == "Rte":
                    ...
                    rec.ei_amount_total_no_withholding = (
                        rec.amount_untaxed + rec.ei_amount_tax_no_withholding
                    )
                    rec.ei_amount_excluded = amount_excluded

                    if self.is_universal_discount():
                        if "ks_global_tax_rate" not in rec:
                    ...
                    if integer_part:
                        ...
                        rec.value_letters = (
                            num2words(integer_part, lang=lang).upper()
                            + " "
    ...
    def get_ei_type_document_id(self):
        self.ensure_one()
        type_documents_env = self.env["l10n_co_edi_shf.type_documents"]
        ...
        for rec in self:
            ...
            if type_edi_document != "none":
                if (
                    ...
                    _logger.debug(
                        "This type of document does not have a DIAN resolution assigned: %s"
                        % rec.name
                    )
            else:
                rec.resolution_id = None
    ...
    @api.depends(
        ...
        "ei_correction_concept_debit_id",
    )
            ...
            if type_edi_document != "none":
                ...
                if rec.get_type_edi_document() == "invoice":
                    json_request["resolution"] = {
                        ...
                        "resolution_date": fields.Date.to_string(
                            rec.resolution_id.resolution_resolution_date
                        ),
                        ...
                        "date_from": fields.Date.to_string(
                            rec.resolution_id.resolution_date_from
                        ),
                        "date_to": fields.Date.to_string(
                            rec.resolution_id.resolution_date_to
                        ),
                    }
                else:
                ...
                if rec.order_ref_number:
                    json_request["order_reference"] = {"number": rec.order_ref_number}
                    if rec.order_ref_date:
                        json_request["order_reference"]["issue_date"] = (
                            fields.Date.to_string(rec.order_ref_date)
                        )
                ...
                if (
                    ...
                    type_currencies_env = self.env["l10n_co_edi_shf.type_currencies"]
                    ...
                    if company_currency_search and invoice_currency_search:
                        ...
                        if (
                            hasattr(rec.currency_id, "rate_inverted")
                            and rec.currency_id.rate_inverted
                        ):
                        ...
                        raise UserError(
                            _(
                                "A currency type in Odoo does not correspond to any DIAN currency type"
                            )
                        )
                ...
                invoice_rec = None
                if type_edi_document != "none":
                    json_request["legal_monetary_totals"] = (
                        rec.get_ei_legal_monetary_totals()
                    )
                        ...
                        if invoice_rec and invoice_rec.ei_uuid:
                            json_request["reference"] = {
                                ...
                                "issue_date": fields.Date.to_string(
                                    invoice_rec.ei_issue_date
                                ),
                            }
                        else:
                            raise UserError(
                                _(
                                    "The reference invoice has not yet been validated before the DIAN"
                                )
                            )
                    elif type_edi_document == "note_support":
                        raise UserError(
                            _(
                                "The credit note for document support cannot be a correction without reference"
                            )
                        )
    ...
    def get_type_document(self):
        ...
        for rec in self:
            if rec.move_type in ("out_invoice", "in_invoice"):
                if (
                    ("debit_origin_id" in rec) and rec.debit_origin_id
        ...
        for rec in self:
            ...
            try:
                if rec.state == "draft":
                    raise UserError(
                        _(
                            "The invoice must first be validated in Odoo, before being sent to the DIAN."
                        )
                    )
                ...
                if (
                    type_edi_document != "none"
                    and not rec.ei_is_valid
                    and not rec.is_journal_pos()
                ):
                    ...
                    api_url = (
                        ...
                        .get_param(
                            "shf.edipo.api_url",
                            "https://shf.apifacturacionelectronica.xyz/api/ubl2.1",
                        )
                    )
                    params = {"token": token}
                    header = {
                    ...
                    if is_test or not rec.ei_is_not_test:
                        if type_edi_document in ("doc_support", "note_support"):
                            raise UserError(
                                _(
                                    "The support document does not support test submissions, only production."
                                )
                            )
                    ...
                    num_attemps = int(
                        self.env["ir.config_parameter"]
                        .sudo()
                        .get_param("shf.edipo.num_attemps", "2")
                    )
                    if is_test or not rec.ei_is_not_test:
                        num_attemps = 1
                        ...
                        if response["is_valid"]:
                            ...
                            if response["uuid"] != "":
                                if not rec.ei_is_not_test:
                                    ...
                                    temp_message = {
                                        rec.ei_status_message,
                                        rec.ei_errors_messages,
                                        rec.ei_status_description,
                                        rec.ei_status_code,
                                    }
                ...
                if not rec.company_id.ei_always_validate:
                    raise UserError(
                        _("Failed to process the request for document: %s: %s")
                        % (rec.name, e)
                    )
                else:
                    rec.message_post(
                        body=_(
                            ...
                            "Failed to process the request for document: %s: %s"
                        )
                        % (rec.name, e)
                    )
            ...
            if (
                not rec.is_edi_mail_sent
                and rec.company_id.enable_mass_send_print
                and rec.is_to_send_edi_email()
            ):
                try:
                    rec.mass_send_print()
                except Exception:
    ...
    def _post(self, soft=True):
        ...
        to_edi = self.filtered(
            lambda inv: inv.company_id.ei_enable
        ...
        if to_edi:
            ...
            if to_paid_invoices:
                raise UserError(
                    _(
                        "Please check your invoice again. Are you really billing something?"
                    )
                )

            # Validate invoices
            to_electronic_invoices = to_edi.filtered(
                lambda inv: inv.state == "posted"
                and not inv.company_id.enable_validate_state
            )
            if to_electronic_invoices:
                to_electronic_invoices.filtered(
                    lambda inv: inv.write(
                            ...
                            for log in response:
                                if log["is_valid"]:
                                    ...
                                    if json_request["issue_date"]:
                                        ...
                                        rec.ei_issue_datetime = json_request[
                                            "issue_date"
                                        ]
                                    ...
                                    if json_request["xml_base64_bytes"]:
                                        rec.ei_xml_base64_bytes = json_request[
                                            "xml_base64_bytes"
                                        ]
                                    ...
                                    if json_request[
                                        ...
                                        rec.ei_application_response_base64_bytes = (
                                            json_request[
                                                "application_response_base64_bytes"
                                            ]
                                        )
                                    if json_request["attached_document_base64_bytes"]:
                                        rec.ei_attached_document_base64_bytes = (
                                            json_request[
                                                "attached_document_base64_bytes"
                                            ]
                                        )
                                    if json_request["pdf_base64_bytes"]:
                                        rec.ei_pdf_base64_bytes = json_request[
                                            "pdf_base64_bytes"
                                        ]
                                    if json_request["zip_base64_bytes"]:
                                        rec.ei_zip_base64_bytes = json_request[
                                            "zip_base64_bytes"
                                        ]
    ...
    def _is_attached_document_matched(self):
        for rec in self:
            ...
            if rec.ei_attached_document_base64_bytes:
                with BytesIO(
                    base64.b64decode(rec.ei_attached_document_base64_bytes)
                ) as file:
                    search_ok = False
                    for line in file:
                        search_string = (
    ...
    def message_update(self, msg_dict, update_vals=None):
        ...
        for rec in self:
            ...
            _logger.debug(
                "Mail event. Invoice: %s, Event: %s" % (rec.number_formatted, rec.event)
            )

        return res

    def create_radian_default_events(self):
        search_env = self.env["l10n_co_edi_shf.radian"]

        for rec in self:
            if (
                rec.move_type in ("out_invoice", "out_refund")
                and rec.payment_form_id.id == 2
            ):
                event_type = "customer"

                # Tacit acceptance
                search_rec = search_env.search(
                    [("move_id", "=", rec.id), ("event_id", "=", 7)]
                )
                if not search_rec:
                    search_env.create(
                        {
