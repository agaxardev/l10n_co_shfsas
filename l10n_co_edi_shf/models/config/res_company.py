...
class ResCompany(models.Model):
    ...
    type_document_identification_id = fields.Many2one(
        comodel_name="l10n_co_edi_shf.type_document_identifications",
    ...
    type_organization_id = fields.Many2one(
        comodel_name="l10n_co_edi_shf.type_organizations",
        compute="_compute_edi",
        inverse="_inverse_type_organization_id",
        string="Organization type",
    )
    type_regime_id = fields.Many2one(
        "l10n_co_edi_shf.type_regimes",
        compute="_compute_edi",
        inverse="_inverse_type_regime_id",
        string="Regime type",
    )
    type_liability_id = fields.Many2one(
        "l10n_co_edi_shf.type_liabilities",
        compute="_compute_edi",
        inverse="_inverse_type_liability_id",
    ...
    merchant_registration = fields.Char(
        string="Merchant registration",
        compute="_compute_merchant_registration",
        store=True,
    )
    ...
    ei_always_validate = fields.Boolean(
        string="Always validate invoices", default=False
    )

    # Test
    is_not_test = fields.Boolean(string="Production environment", default=False)
    test_set_id = fields.Char(string="TestSetId")
    enable_validate_state = fields.Boolean(
        string="Enable intermediate 'DIAN Validation' state", default=True
    )
    enable_mass_send_print = fields.Boolean(
        string="Automatic invoice email when validating (In production)", default=False
    )
    ...
    ei_include_pdf_attachment = fields.Boolean(
        string="Include PDF attachment on electronic invoice email", default=True
    )

    # Enable/disable electronic invoicing for company
    ei_enable = fields.Boolean(
        string="Enable electronic invoicing for this company", default=True
    )

    # Ignore email edi
    ei_ignore_edi_email_check = fields.Boolean(
        string="Ignore edi email check", default=False
    )
        ...
        for partner in self:
            if partner.email_edi:
                partner.email_edi_formatted = tools.formataddr(
                    (
                        (partner.name + _(" - Electronic invoicing")) or "False",
                        partner.email_edi or "False",
                    )
                )
            else:
    ...
    def get_type_document_identification_id(self):
        for rec in self:
            document_type = (
                rec.partner_id.l10n_latam_identification_type_id.l10n_co_document_code
            )
            if document_type:
                values = {
    ...
    def _inverse_type_document_identification_id(self):
        for company in self:
            ...
            l10n_latam_identification_type_rec = self.env[
                "l10n_latam.identification.type"
            ].search([("l10n_co_document_code", "=", l10n_co_document_code)])[0]
            company.partner_id.l10n_latam_identification_type_id = (
                l10n_latam_identification_type_rec.id
            )
    ...
    def update_environment(self, environment):
        ...
        for rec in self:
            ...
            try:
                ...
                response = requests.get(api_url, headers=header, params=params).json()
                ...
                if "type_environment_id" in response:
                    if environment == response["type_environment_id"]:
                        ...
                        _logger.debug(
                            "The environment has been updated. Sync the resolutions"
                        )
                        success = True
