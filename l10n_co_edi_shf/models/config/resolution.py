...
class Resolution(models.Model):
    _name = "l10n_co_edi_shf.resolution"
    ...
    resolution_type_document_id = fields.Many2one(
    ...
    resolution_id = fields.Integer(
        string="Api ID", readonly=True, copy=False, index=True
    )
    resolution_number = fields.Integer(string="Number", readonly=True, copy=False)
    resolution_next_consecutive = fields.Char(
        string="Next consecutive", readonly=True, copy=False
    )
    ...
    def post_resolution(self, vals):
        success = False
        try:
            requests_data = {"code": vals["resolution_type_document_id"]}
            ...
            _logger.debug(
                "Request create resolution DIAN: %s",
            ...
            if "resolution" in response:
                ...
                vals["resolution_next_consecutive"] = response["resolution"][
                    "next_consecutive"
                ]
                success = True
            ...
            try:
                ...
                requests_data = {
                    ...
                    "resolution_date": fields.Date.to_string(
                        rec.resolution_resolution_date
                    ),
                ...
                _logger.debug(
                    "Request update resolution DIAN: %s",
                    json.dumps(requests_data, indent=2, sort_keys=False),
                )
        ...
        for rec in self:
            try:
                ...
                response = requests.delete(
                    api_url, headers=header, params=params
                ).json()
                _logger.debug("API Response: %s", response)
