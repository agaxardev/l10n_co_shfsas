...
class ResPartner(models.Model):
    ...
    type_regime_id = fields.Many2one(comodel_name='l10n_co_edi_shf.type_regimes', string="Regime type",
                                     ondelete='RESTRICT')
    type_liability_id = fields.Many2one(comodel_name='l10n_co_edi_shf.type_liabilities',
                                        string="Liability type", ondelete='RESTRICT')
    merchant_registration = fields.Char(string="Merchant registration")
    municipality_id = fields.Many2one(comodel_name='l10n_co_edi_shf.municipalities', string="Municipality",
                                      ondelete='RESTRICT')
    ...
    customer_software_id = fields.Many2one(comodel_name='l10n_co_edi_shf.customer_software',
                                           string="Customer software", copy=False, ondelete='RESTRICT')
    type_document_identification_id = fields.Many2one(comodel_name="l10n_co_edi_shf.type_document_identifications",
    ...
    # Postal fields
    postal_id = fields.Many2one(comodel_name='l10n_co_edi_shf.postal', copy=True, string="Postal",
                                compute="_compute_postal_id", store=True)
    postal_department_id = fields.Many2one(comodel_name='l10n_co_edi_shf.departments', copy=True,
                                           string="Postal department", compute="_compute_postal_id", store=True)
    postal_municipality_id = fields.Many2one(comodel_name='l10n_co_edi_shf.municipalities', copy=True,
    ...
    def _compute_type_document_identification_id(self):
        if not self.env['l10n_co_edi_shf.type_document_identifications'].search_count([]):
            self.env['res.company'].init_csv_data('l10n_co_edi_shf.l10n_co_edi_shf.type_document_identifications')
    ...
    def _compute_postal_id(self):
        for rec in self:
            if rec.zip and rec.country_id and rec.country_id.code == 'CO':
                postal_obj = rec.env['l10n_co_edi_shf.postal']
                postal_search = postal_obj.sudo().search([('name', '=', rec.zip)])
                if postal_search:
                    rec.postal_id = postal_search[0].id
                    rec.postal_department_id = rec.env['l10n_co_edi_shf.departments'].sudo().search(
                        [('code', '=', rec.postal_id.department_id.code)]
                    )[0].id
                    rec.postal_municipality_id = rec.env['l10n_co_edi_shf.municipalities'].sudo().search(
